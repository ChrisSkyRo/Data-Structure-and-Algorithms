...
{ //apelam fiecare functie sa vedem daca exista
	...
	c2.adauga( 3 );
	c2.adauga( 7 );
	assert( c2.elementeCuFrecventaData( 1 ) == 2 );
	assert( c2.elementeCuFrecventaData( 2 ) == 1 );
	assert( c2.elementeCuFrecventaData( 19 ) == 0 );
	try
	{
		c2.elementeCuFrecventaData( -3 );
	}
	catch ( std::exception )
	{
		exceptie = true;
