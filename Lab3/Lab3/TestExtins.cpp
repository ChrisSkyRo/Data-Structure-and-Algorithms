...
void testCreeaza()
{
	...
	assert( c.vida() == true );
	for ( int i = -5; i < 5; i++ )
	{ //cautam in colectia vida
		assert( c.cauta( i ) == false );
	}
	for ( int i = -10; i < 10; i++ )
	{ //stergem din colectia vida
		assert( c.sterge( i ) == false );
	}
	for ( int i = -10; i < 10; i++ )
	{ //numaram aparitii in colectia vida
...
void testAdauga()
{
	...
	assert( c.dim() == 10 );
	for ( int i = -10; i < 20; i++ )
	{ //mai adaugam elemente [-10, 20), acum anumite elemente [0, 10) sunt de 2 ori
		c.adauga( i );
	}
	assert( c.vida() == false );
	assert( c.dim() == 40 );
	...
	{ //numaram de cate ori apar anumite elemente (inclusiv elemente inexistente)
...
void testSterge()
{
	...
	for ( int i = -100; i < 100; i++ )
	{ //stergem o aparitie de la fiecare
		assert( c.sterge( i ) == true );
	}
	assert( c.dim() == 800 );
	for ( int i = -100; i < 100; i++ )
	...
	for ( int i = -200; i < 200; i++ )
	{ //stergem 5 aparitii de la elemente inexistente si existente (dar si acolo exista doar 4 aparitii)
		if ( i < -100 || i >= 100 )
		{
			assert( c.sterge( i ) == false );
			assert( c.sterge( i ) == false );
	...
	int max = 200;
	while ( min < max )
	{ //adaugam elemente, pe 0 de 2 ori
		c.adauga( min );
		c.adauga( max );
		min++;
		max--;
	...
	assert( c.dim() == 402 );
	for ( int i = -30; i < 30; i++ )
	{ //stergem o parte dintre elemente
...
void testIterator()
{ // nu stim reprezentarea Colectiei, putem testa doar anumite lucruri generale, nu stim in ce ordine vor fi afisate elementele.
	Colectie c;
	IteratorColectie ic = c.iterator(); //iterator pe colectie vida
	assert( ic.valid() == false );

	for ( int i = 0; i < 100; i++ )
	{  //adaug 100 de elemente, fiecare e valoarea 33
		c.adauga( 33 );
	}
	...
	{ //scoatem pe rand elemente din vectorul dat de iterator, verificam sa fie in colectie si le stergem si din vector si din c4. Incepem stergerea cu ultimul
	...
	for ( unsigned int i = 0; i < elemente2.size(); i++ )
	{ //scoatem pe rand elemente din vectorul dat de iterator, verificam sa fie in colectie si le stergem si din vector si din c4. Incepem stergerea cu primul
...
{//scopul e sa adaugam multe date 
	...
	ic.prim();
	while ( ic.valid() )
	{ //fiecare element returnat de iterator trebuie sa fie in colectie si nrAparitii trebuie sa fie mai mare ca 0
		TElem e = ic.element();
